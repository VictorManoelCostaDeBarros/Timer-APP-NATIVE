...
export default class App extends React.Component{
  ...
  async tocarAlarme(){
    ...
    try{
      await this.soundObject.loadAsync(require('./alarm.mp3'));
    ...
    if(this.state.selecionado == false){
      return (
        <View style={styles.boxTimerSelect}>
          ...
          <Picker style={{width: 200,height: 30,marginTop: 15}} onValueChange={(value,index) => this.setState({selecionado:true,contagem:value})}>
      ...
      return(
        <View style={styles.boxTimerSelect}>
          ...
          <TouchableOpacity style={styles.btn} onPress={()=>this.pressionado()}><Text style={{color: 'white'}}>Começar!</Text></TouchableOpacity>
        </View>
      )
    }
  }
